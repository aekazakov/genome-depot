...
def preprocess(annotator, genomes, working_dir):
    ...
    for genome in genomes:
        ...
        with gzip.open(os.path.join(genome_dir,
                                    genome + '_genomic.gbff.gz'
                                    ), 'wt') as outfile:
    ...
    with open(ecis_screen_script, 'w') as outfile:
        ...
        outfile.write(' '.join([
                      annotator.config['plugins.ecis_screen.ecis-screen_cmd'],
...
def postprocess(annotator, genomes, working_dir):
    """
        Finds eCIS-screen output files and creates file 
    ...
    accessions = {}
    for genome in genomes:
        gbk_file = os.path.join(working_dir,
                                'genomes',
                                genome,
                                genome + '_genomic.gbff.gz'
                                )
            ...
            with open(os.path.join(working_dir,
                ...
                for line in infile:
                    ...
                    try:
                        ...
                        print ('Unable to find genome name for sequence ID ' +
                               row[0])
                        raise
                    for ind, genes in enumerate(row):
                        ...
                        if genes == '-':
                            continue
                        family = header[ind]
                        description = family + ' subunit of extracellular ' + \
                                      'contractile injection system (eCIS).'
