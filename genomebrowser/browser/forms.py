...
class GenomeImportForm(forms.Form):
    import_tsv_file = forms.FileField(
        label='Choose the tab-separated text file',
        required=False
        )


class GenomeUploadForm(forms.Form):
    upload_tsv_file = forms.FileField(label='Choose the tab-separated text file',
        required=False,
        )
...
class GenomeDownloadForm(forms.Form):
    download_tsv_file = forms.FileField(label='Choose the tab-separated text file',
        required=False,
        )
...
class ChooseAnnotationToolForm(forms.Form):
    # The tools field is empty on the form initialization
    # to prevent calling db server on startup
