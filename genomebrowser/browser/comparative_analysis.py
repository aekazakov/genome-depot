...
def _get_color(index):
    """
        Returns a color gradient as a string for an index of the COLORS list
    """
...
def add_gene(gene, gene_uid, track_uid, offset, reverse_gene,
    ...
    """
        Returns javascript code for a gene glyph in Scribl 
        with all gene attributes
    """
    result = []
    if gene.strand == 1:
        if reverse_gene:
...
def make_muscle_alignment(infasta):
    """
        Returns MSA in FASTA format 
        generated by MUSCLE
        
        In:
        str: proteins in FASTA format
    """
    result = None
    muscle_cline = 'muscle'
...
def make_protein_tree(proteins):
    """
        Returns a list of nodes, a phylogenetic tree in HTML
        format, a phylogenetic tree in Newick format
