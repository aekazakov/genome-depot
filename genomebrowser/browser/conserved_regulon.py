...
def build_conserved_regulon(og_id):
    ...
    regulons = Regulon.objects.filter(regulators__id__in=
                                      [item.id for item in regulators]
                                      ).select_related(
                                        'genome',
                                        'genome__taxon'
                                      ).prefetch_related(
    ...
    sites = Site.objects.filter(regulon__id__in=
                                [item.id for item in regulons
                                ]).select_related(
                                    'genome',
                                    'contig',
                                    'regulon'
                                    ...
                                    'operons__contig',
                                    'operons__genes',
                                    'operons__genes__genome',
                                    'operons__genes__protein',
                                    'operons__genes__protein__ortholog_groups',
                                    'operons__genes__protein__ortholog_groups__taxon',
                                    'genes__protein',
                                    'genes__protein__ortholog_groups',
                                    'genes__protein__ortholog_groups__taxon'
                                    )
    ...
    for site in sites:
        ...
        for gene in site.genes.all():
            ...
            if gene.protein is None:
                ...
                fake_id -= 1
                eggnogs[current_id][regulon_label].append((gene.locus_tag,
                                                           gene.genome.name,
                                                           gene.function
                                                           ))
                ...
                if eggnog_group is not None:
                    eggnogs[eggnog_group.id][regulon_label].append((gene.locus_tag,
                                                                    gene.genome.name,
                                                                    gene.function
                                                                    ))
                    ...
                    eggnog_labels[eggnog_group.id] = eggnog_group.eggnog_id + \
                                                     '[' + eggnog_group.taxon.name + ']'
                else:
                    current_id = str(fake_id)
                    fake_id -= 1
                    eggnogs[current_id][regulon_label].append((gene.locus_tag,
        ...
        for operon in site.operons.all():
            ...
            for gene in operon.genes.all():
                ...
                if not gene.protein:
                    ...
                    eggnog_group = get_lowest_level_og(gene.protein)
                    if eggnog_group is not None:
                        eggnogs[eggnog_group.id][regulon_label].append((
                            gene.locus_tag,
                            gene.genome.name,
                            gene.function
                            ))
                        if gene.function not in eggnog_functions[eggnog_group.id]:
                            eggnog_functions[eggnog_group.id][gene.function] = 0
                        eggnog_functions[eggnog_group.id][gene.function] += 1
                        eggnog_labels[eggnog_group.id] = \
                            eggnog_group.eggnog_id + '[' + \
                            eggnog_group.taxon.name + ']'
                    else:
                        current_id = str(fake_id)
                        fake_id -= 1
    ...
    for eggnog_id in eggnogs:
        ...
        if eggnog_id in eggnog_labels:
            ...
            table_row = ['<td class="sticky-col style3">' +
                         '<a href="javascript:alert(\'No EggNOG ID.\');"' +
                         'title="No EggNOG ID">' + max_function + '</a>' + '</td>'
                         ]
        ...
        for regulon_label in sorted(list(regulon_labels)):
            if regulon_label in eggnogs[eggnog_id]:
                ...
                for index,item in enumerate(eggnogs[eggnog_id][regulon_label]):
                    tooltip.append(str(index + 1) +
                                   '. Gene: <a href="' +
                                   reverse('genedetails', args=(item[1], item[0])) +
                                   '" title="' +
                                   item[0] + ': ' +
                                   item[2] + '">' +
                                   item[0] + '</a> [' +
                ...
                table_row.append('<td class="filledcell"><div class="tooltip">' +
                                 str(len(eggnogs[eggnog_id][regulon_label])) +
                                 '<span>' + '<br/>'.join(tooltip) +
                                 '</span></div></td>'
                                 )
            else:
