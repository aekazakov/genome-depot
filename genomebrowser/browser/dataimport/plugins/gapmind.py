...
def application(annotator, genomes):
    ...
    """
    working_dir = os.path.join(annotator.config['cgcms.temp_dir'], 'gapmind-plugin-temp')
...
def preprocess(annotator, genomes, working_dir):
    ...
    with open(gapmind_script, 'w') as outfile:
        ...
        outfile.write('conda activate ' + annotator.config['plugins.gapmind.gapmind_env'] + '\n')
        ...
        for genome in sorted(genomes.keys()):
            ...
            outfile.write('\n' + ' '.join(['perl', 'bin/buildorgs.pl', '-out', os.path.join(output_dir, genome, 'orgs'),
                                    '-orgs', "'file:" + input_fasta_files[genome] + ":" + genome + "'"]) + '\n')
            # diamond makedb --in $TEMPDIR/orgs.faa --db $TEMPDIR/orgs.faa
            outfile.write(' '.join(['diamond', 'makedb', '--in', os.path.join(output_dir, genome, 'orgs.faa'),
                                    '--db', os.path.join(output_dir, genome, 'orgs.faa')
                                    ]) + '\n')
                                    
            for collection in GAPMIND_REFERENCE:
                # perl bin/gapsearch.pl -orgs $TEMPDIR/orgs -set aa -out $TEMPDIR/aa.hits -nCPU 8
                outfile.write(' '.join(['perl', 'bin/gapsearch.pl', '-orgs', os.path.join(output_dir, genome, 'orgs'), '-set', collection, 
                                        '-out', os.path.join(output_dir, genome, collection + '.hits'), '-nCPU', annotator.config['plugins.gapmind.threads']
                                        ]) + '\n')
                # perl bin/gaprevsearch.pl -orgs $TEMPDIR/orgs -hits $TEMPDIR/aa.hits -curated tmp/path.aa/curated.faa.udb.dmnd -out $TEMPDIR/aa.revhits -nCPU 8
                outfile.write(' '.join(['perl', 'bin/gaprevsearch.pl', '-orgs', os.path.join(output_dir, genome, 'orgs'),
                                        '-hits', os.path.join(output_dir, genome, collection + '.hits'),
                                        '-curated', os.path.join(annotator.config['plugins.gapmind.gapmind_dir'], 'tmp', 'path.' + collection, 'curated.faa.udb.dmnd'),
                                        '-out', os.path.join(output_dir, genome, collection + '.revhits'), '-nCPU', annotator.config['plugins.gapmind.threads']
                                        ]) + '\n')
                # perl bin/gapsummary.pl -orgs $TEMPDIR/orgs -set aa -hits $TEMPDIR/aa.hits -rev $TEMPDIR/aa.revhits -out $TEMPDIR/aa.sum
                outfile.write(' '.join(['perl', 'bin/gapsummary.pl', '-orgs', os.path.join(output_dir, genome, 'orgs'), '-set', collection,
                                        '-hits', os.path.join(output_dir, genome, collection + '.hits'),
                                        '-rev', os.path.join(output_dir, genome, collection + '.revhits'),
                                        '-out', os.path.join(output_dir, genome, collection + '.sum')
                                        ]) + '\n')
...
def postprocess(annotator, genomes, working_dir):
    ...
    """
    output_file = os.path.join(annotator.config['cgcms.temp_dir'], 'gapmind-plugin-output.txt')
    with open(output_file, 'w') as outfile:
        for collection in GAPMIND_REFERENCE:
            ref_data = defaultdict(dict)
            # Read GapMind reference data
            with open(os.path.join(annotator.config['plugins.gapmind.gapmind_dir'], collection + '.ref.txt'), 'r') as infile:
                for line in infile:
                    row = line.rstrip('\n\r').split('\t')
                    ref_data[row[0]]['pathway'] = row[1]
                    ref_data[row[0]][row[2]] = row[3]
            for genome in genomes:
                cand_outfile = os.path.join(working_dir, 'out', genome, collection + '.sum.cand')
                if not os.path.exists(cand_outfile):
                    print('File does not exist:', cand_outfile)
                    continue
                rules_outfile = os.path.join(working_dir, 'out', genome, collection + '.sum.rules')
                ...
                with open(cand_outfile, 'r') as infile:
                    ...
                    for line in infile:
                        ...
                        if score == '0':
                            continue
                        locus_tag = row[6]
                        outfile.write('\t'.join([locus_tag, genome, 'GapMind',
