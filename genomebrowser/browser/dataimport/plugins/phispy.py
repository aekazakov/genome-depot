...
def application(annotator, genomes):
    ...
    """
    working_dir = os.path.join(annotator.config['cgcms.temp_dir'], 'phispy-plugin-temp')
    ...
    with open(phispy_script, 'w') as outfile:
        ...
        for genome in sorted(genomes.keys()):
            outfile.write(' '.join(['PhiSpy.py',
                                    '-o', os.path.join(output_dir, genome),
                                    '--threads', annotator.config['plugins.phispy.threads'],
        ...
        for genome in input_fasta_files:
            ...
            outfile.write(' '.join(['hmmsearch',
                  '--domtblout', domfile, '-o', '/dev/null',
                  '-E', annotator.config['plugins.phispy.evalue_threshold'],
                  '--cpu', annotator.config['plugins.phispy.threads'], '--noali', '--notextw',
...
def postprocess(annotator, genomes, working_dir):
    ...
    """
    output_file = os.path.join(annotator.config['cgcms.temp_dir'], 'phispy-plugin-output.txt')
    with open(output_file, 'w') as outfile:
        outfile.write('#Gene\tGenome\tSoure\tURL\tKey\tValue\tNote\n')
        for genome in sorted(genomes.keys()):
            phispy_regions = os.path.join(working_dir, 'out', genome, 'prophage_coordinates.tsv')
            phispy_genes = os.path.join(working_dir, 'out', genome, 'prophage_information.tsv')
            ...
            domains = parse_hmmsearch_output(os.path.join(working_dir, 'out', genome + '.domtblout.txt'), annotator.config['plugins.phispy.evalue_threshold'])
                ...
                if gene in domains:
                    for hmm in sorted(domains[gene].keys()):
                        outfile.write('\t'.join([
                            ...
                            genome,
                            'pVOGs',
                            'http://dmk-brain.ecn.uiowa.edu/pVOGs/VOGtables/' + hmm + '.html',
                            'Prophage gene',
                            hmm,
                            hmm + ' virus orthologous group. E-value: ' + domains[gene][hmm]['evalue'] + '. Coordinates: ' + ';'.join(domains[gene][hmm]['coords']) + '. Predicted prophage region ' + genes[gene]
                            ]) + '\n')
