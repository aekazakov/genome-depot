...
def application(annotator, genomes):
    ...
    """
    working_dir = os.path.join(annotator.config['cgcms.temp_dir'], 'amrfinder-plugin-temp')
...
def preprocess(annotator, genomes, working_dir):
    ...
    with open(amrfinder_script, 'w') as outfile:
        ...
        outfile.write('source ' + annotator.config['plugins.amrfinder.conda_path'] + '\n')
        outfile.write('conda activate ' + annotator.config['plugins.amrfinder.conda_env'] + '\n')
...
def postprocess(annotator, genomes, working_dir):
    """
        Finds amrfinder output files and creates file with annotations for upload into DB
    """
    output_file = os.path.join(annotator.config['cgcms.temp_dir'], 'amrfinder-plugin-output.txt')
    with open(output_file, 'w') as outfile:
        outfile.write('#Gene\tGenome\tSoure\tURL\tKey\tValue\tNote\n')
        for genome in sorted(genomes.keys()):
            amrfinder_output = os.path.join(working_dir, 'out', genome + '.amrfinder.tsv')
            with open(amrfinder_output, 'r') as infile:
                ...
                for line in infile:
                    ...
                    description = row[2] + ' [' + row[4] + '/' + row[5] + '/' + row[6] + '/' + row[7] + ']'
