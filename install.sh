...
#Install MacsyFinder
if conda env list | grep 'cgcms-macsyfinder' >/dev/null 2>&1; then
    echo "Found cgcms-macsyfinder environment"
    conda activate cgcms-macsyfinder
    if macsyfinder --version|grep "Macsyfinder" >/dev/null 2>&1; then
        echo "MacsyFinder found"
    else
        echo 'Conda environment cgcms-macsyfinder exists but MacsyFinder was not properly installed. Remove the environment and restart CGCMS installation script.'
        echo 'To remove the environment, run:'
        echo '   conda remove -n cgcms-macsyfinder --all'
        conda deactivate
        exit 1
    fi
    conda deactivate
    ...
    conda create -y -n cgcms-macsyfinder
    conda activate cgcms-macsyfinder
    conda install -y -c bioconda macsyfinder
    mkdir "$CGCMSDIR/external_refdata/macsyfinder"
    mkdir "$CGCMSDIR/external_refdata/macsyfinder/data"
    macsydata install --target "$CGCMSDIR/external_refdata/macsyfinder/data" TXSScan
    conda deactivate
    cd "$CGCMSDIR/external_tools"
fi
